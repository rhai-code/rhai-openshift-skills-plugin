...
- **`pkg/api/`** - REST handlers:
  - `auth.go` - Authentication and authorization middleware:
    ...
    - `CheckUserNamespaceAccess(user, namespace)` - SubjectAccessReview (not SelfSubjectAccessReview) checking whether the *user* can create, exec into, and delete pods in a namespace. Used by `CreateScheduledTask` and `UpdateScheduledTask` to prevent privilege escalation when a container image is specified. Permissive in dev mode (no kube client).
  ...
  - `schedule.go` - Task scheduler supporting both cron (robfig/cron) and run-once (`time.AfterFunc`) execution:
    ...
    - **No container image** → `executeLLMTask()`: agent loop with shell commands exec'd into agent-shell sidecar (no kube credentials) → stores results in chat session
    ...
    - **User namespace authorization**: `CreateScheduledTask` and `UpdateScheduledTask` call `CheckUserNamespaceAccess()` when a container image is set, performing SubjectAccessReview against the *user's* identity to verify they can create/exec/delete pods in the target namespace. Returns 403 with specific missing permissions if denied. This prevents privilege escalation where a user specifies a privileged namespace/SA and the plugin SA creates the pod on their behalf.
    - `CheckSchedulePermissions` (GET) - uses `SelfSubjectAccessReview` to check if plugin SA can create/exec/delete pods in a namespace. Returns `allowed` boolean, `missing` permissions list, and `oc_command` to create the required RoleBinding. Frontend calls this when container image is set. This is a separate informational check — the authoritative gate is the user-level SAR in create/update.
  ...
  - `maas_endpoints.go` - CRUD for MaaS endpoints, model listing, health checks. List returns `is_global=1 OR owner=?` for non-admins (admins see all). Create sets owner, accepts `is_global`. Update uses partial updates (pointer fields, per-field SQL) so toggling `is_global` alone won't blank other fields. Update/delete check ownership. ListModels scoped to visible endpoints. Supports two endpoint types:
...
- **`deployment.yaml`** - Two-container pod: `plugin` (Go backend, projected SA token at `/var/run/kube-sa/`, serves API and manages executor pods, PVC for SQLite data) and `agent-shell` sidecar (`sleep infinity`, no SA token, no kube access, no database PVC — LLM agent shell commands exec into here). Sets `POD_NAMESPACE`, `POD_NAME`, `POD_SERVICE_ACCOUNT`, `PLUGIN_NAME`, `AGENT_SHELL_CONTAINER` via downward API/values. `automountServiceAccountToken: false` with explicit projected token volume. TLS from serving cert secret, `MLFLOW_TRACKING_URI` env var (when mlflow enabled)
- **`rbac.yaml`** - Namespace-scoped Role for secrets get (maas-secret auto-seed). ClusterRole for namespace get only (namespace annotation reading for token expiry). Namespace-scoped Role for pods create/delete, pods/log get, pods/exec create (plugin namespace only — admin must create additional RoleBindings for the plugin SA in other namespaces where users want to run executor pods). ClusterRole/Binding for plugin SA to create TokenReviews and SubjectAccessReviews (RBAC auth). User-facing ClusterRoles: `{plugin-name}-user` (verb `use` on `skills.openshift.io/plugins`) and `{plugin-name}-admin` (verbs `use`, `admin`). Bind to users via `oc adm policy add-cluster-role-to-user`.
...
- **Agent shell isolation**: The plugin pod has two containers. The `plugin` container runs the Go backend with a projected SA token (at `/var/run/kube-sa/`, not the default path) for kube API access and the SQLite data PVC mounted. The `agent-shell` sidecar runs `sleep infinity` with `automountServiceAccountToken: false`, no token volume, and no data PVC — all LLM agent shell commands (chat and LLM-only scheduled tasks) exec into this container via SPDY. This prevents the LLM from accessing the kube API, reading secrets, reading the application database (API keys, session data), or performing privileged operations even if it attempts to construct its own kubeconfig.
...
- **Multi-tenancy and RBAC**: Per-user data ownership and role-based access control using OpenShift RBAC primitives:
  ...
  - **Scheduled task namespace authorization**: When a user creates or updates a task with a container image, `CheckUserNamespaceAccess()` performs a SubjectAccessReview against the user's identity to verify they have pod create/exec/delete permissions in the target namespace. Without this, a user could specify a privileged namespace and service account, and the plugin SA would create the executor pod on their behalf — a privilege escalation vector. The check mirrors the same three permissions that `CheckPodPermissions` checks for the plugin SA, but enforced against the user at task write time.
