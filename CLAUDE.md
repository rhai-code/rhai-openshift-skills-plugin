...
- **`pkg/api/`** - REST handlers:
  ...
  - `schedule.go` - Task scheduler supporting both cron (robfig/cron) and run-once (`time.AfterFunc`) execution:
    ...
    - `CheckSchedulePermissions` (GET) - uses `SelfSubjectAccessReview` to check if plugin SA can create/exec/delete pods in a namespace. Returns `allowed` boolean, `missing` permissions list, and `oc_command` to create the required RoleBinding. Frontend calls this when container image is set.
